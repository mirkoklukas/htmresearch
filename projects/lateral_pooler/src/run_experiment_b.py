...
def dump_pooler(path, t, pooler):
    filename = "{}/pooler_{:04}.p".format(path, t)
...
def parse_argv():
    parser = OptionParser(usage = "<yourscript> [options]\n\n"\
                                  "Example:\n"\
                                  "python {} --sp lateral --data mnist --params path_to_dict --exp path_to_folder -e 2 -b 1 -d 100"
                                  .format(sys.argv[0]))
    ...
    parser.add_option("--params", type=str, dest="params_path", help="json file with spatial pooler parameters")
    parser.add_option("--exp", type=str, default=None, dest="experiment_path", help="")
...
def main(argv):
    ...
    ####################################################
    if sp_type == "nupic":
        for key in ["lateralLearningRate","enforceDesiredWeight","lateralDutyCyclePeriod"]:
            if key in sp_params:
                del sp_params[key]
        ...
        raise "I don't know a SP of that type:{}".format(sp_type)
